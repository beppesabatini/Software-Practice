...
public class MaxHourglassSumFinder {
	 ...
	 * the highest sum. In a 6x6 matrix there are sixteen possible hourglass
	 * patterns. A sample hourglass pattern:
	 * 
	 * <pre>
	 *    2 4 4
	 *      2 
	...
	public static int hourglassSum(int[][] currentMatrix) {
		String dimensionsError = String.format("Bad input: current matrices must be %d x %d", MATRIX_HEIGHT,
				MATRIX_WIDTH);
		if (currentMatrix.length != MATRIX_HEIGHT) {
			System.out.println(dimensionsError);
			return (-1);
		}
