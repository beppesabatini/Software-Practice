...
/**
 * From Java Examples in a Nutshell, 3rd Edition, pp. 325-330. This class
 * extends ResourceBundle and adds methods to retrieve types of resources
 * commonly used in GUIs. Additionally, it adds extensibility by allowing
 * ResourceParser objects to be registered to parse other resource types. This
 * class is used by the WebBrowser in the current package. It cannot be run by
 * itself at this time, but testing the WebBrowser will at least partly test it.
 */
public class GUIResourceBundle extends ResourceBundle {
	...
	/*
	 * The resource bundle that actually contains the textual resources This class
	 * is a wrapper around this bundle
	 */
	 ...
	 * constructor takes advantage of the internationalization features of the
	...
	/**
	 * Create a PropertyResourceBundle from the specified stream, and then create a
	 * GUIResourceBundle wrapper for it.
	 **/
	...
	/**
	 * Create a PropertyResourceBundle from the specified properties file, and then
	...
	/**
	 * This method is like the inherited getString() method, except that when the
	 * named resource is not found, it returns the specified default instead of
	...
	/**
	 * Look up the named resource and parse it as a list of strings separated by
	...
	public Font getFont(String key) throws MissingResourceException {
		/*
		 * Font.decode() always returns a Font object, so we can't check whether the
		 * resource value was well-formed or not.
		 */
	...
	static HashMap<Class<?>, ResourceParser> parsers = new HashMap<Class<?>, ResourceParser>();
	...
	/**
	 * Look for a ResourceParser for the named type, and if one is found, ask it to
	 * parse and return the named resource
	 **/
	public Object getResource(String key, Class<?> type) throws MissingResourceException {
		// Get a parser for the specified type
		ResourceParser parser = parsers.get(type);
		if (parser == null) {
			throw new MissingResourceException("No ResourceParser registered for " + type.getName() + " resources",
					type.getName(), key);
		}
		...
		try { // Ask the parser to parse the resource
			...
			/*
			 * If any other type of exception occurs, convert it to a
	...
	public static class Demo {
		public static void main(String[] args) {
			Class<GUIResourceBundle> dummy = GUIResourceBundle.class;
			GUIResourceBundle guiResourceBundle = new GUIResourceBundle(dummy, "je3.ch11.gui.WebBrowserResources");
			Enumeration<String> keyDispenser = guiResourceBundle.getKeys();
			while(keyDispenser.hasMoreElements()) {
				String key = keyDispenser.nextElement();
				String property = guiResourceBundle.getString(key);
