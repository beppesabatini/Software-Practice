<%-- ---------------------------------------------------------------------
  - From Java Examples in a Nutshell, 3rd Edition, pp. 597-598. 
...
<html>
  <head>
    <title>ListManager: Delivery Preferences</title>
  <head>
    <body>
      <div align="center">
        <h1>List Manager: ${applicationScope.listname}</h1>
        <tags:box bgcolor="#ccf" border="3" margin="75" padding="20">
        <!-- This table puts the edit form on the left, -->
        <!--  and stacks the unsubscribe and logout buttons on the right. -->
        <table>
          <tr>
            <td>
              <!-- Custom tag creates a titled box for the edit form --> 
              <tags:box bgcolor="#fff" border="1" margin="25" padding="20" title="Preferences for ${sessionScope.user.emailAddress}">
                <!-- The edit form -->
                <form action="edit.action" method="post">
                  ...
                  <input type="checkbox" name="html" ${sessionScope.user.prefersHTML?"checked":""}/>
                  <p/>
                  Send digests: <input type="checkbox" name="digest" ${sessionScope.user.prefersDigests?"checked":""} />
                  <p/>
            ...
            <td>
              ...
              <form action="unsubscribe.action" method="post">
                <input type="submit" value="Unsubscribe ${sessionScope.user.emailAddress}">
              </form>
              <p/>
              <!-- The logout form -->
