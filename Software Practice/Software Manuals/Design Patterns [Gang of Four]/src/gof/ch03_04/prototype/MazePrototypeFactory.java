 ...
 * example of the {@linkplain gof.designpatterns.Prototype Prototype} design
 * pattern. This class will work badly or not at all. The manual uses a bad
 * example in this case. The Prototype pattern will only work for simpler data
 * types.
 * <p/>
 * The Prototype pattern depends on being able to clone prototypes, sample
 * classes which can be swapped out and exchanged for different samples during
 * runtime. But, in a Maze such as the type we have been working with, most or
 * all rooms will have a pointer to another room. So to create a fully
 * well-formed clone of one single room, or even one single door, means, in
 * effect, to clone the entire maze.
 * <p/>
